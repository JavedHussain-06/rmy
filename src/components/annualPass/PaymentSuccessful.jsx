...
const PaymentSuccessful = () => {
  const { paymentStatus, numberPlate, setShowCongratulations } = useDataStore();

  const handleContinue = () => {
    setShowCongratulations(true); // ✅ this will replace the onboarding page with CongratulationsPage
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-start p-8 pb-10 font-inter bg-white">
      ...
      <div className="max-w-md px-4">
        <p className="text-[#212529] text-[1rem] leading-relaxed">
          ...
          <span className="font-bold">{numberPlate}</span> is <br />
          now being issued.
        </p>
      </div>
      ...
      <div className="border-t border-b border-dashed border-[#DCDEE0] my-6 w-full max-w-md px-6 py-4 text-sm text-gray-700 space-y-4">
        {[
          ["Reference Number", paymentStatus?.response?.razorpay_payment_id || "—"],
          ["Date", new Date().toLocaleDateString()],
          ["Time", new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })],
          ["Payment Method", "UPI"], // optional: dynamic if available
        ].map(([label, value]) => (
          <div
            key={label}
      ...
      <div className="w-full max-w-md px-6 mt-6">
        <button
          className="w-full bg-darkBlue text-white py-3 tracking-[0.04rem] cursor-pointer rounded-[0.7rem] text-[1rem] font-semibold"
